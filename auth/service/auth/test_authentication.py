...
def test_login(storage, helpers):
    test_cases = [
        {"name": "pass", "code": 200, "reason": ""},
        {"name": "invalid", "code": 401, "reason": "invalid password"},
        {"name": "fail", "code": 500, "reason": "failed to login"},
    ]
    for test_case in test_cases:
        client = helpers.authentication(storage=storage)
        client.storage.find_user.return_value = (
            test_case["code"],
            test_case["reason"],
            MockPerson(),
        )
...
def test_validate_token(storage, helpers):
    ...
    for test_case in test_cases:
        ...
        output = client.validate_token(
            ValidateTokenRequest(
                "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwiZW1haWwiOiJva2lkaWppbW1pZTEyMzQ1NjdAZ21haWwuY29tIn0.Co2NFMS1-PdA_8d9ZHvGBWMltUQt3xKj8IganUlNXbE"
            )
        )
        assert output.code == test_case["code"]
        assert output.reason == test_case["reason"]
