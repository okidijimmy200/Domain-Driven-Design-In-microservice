...
class Authentication(AuthenticationInterface):
    ...
    def login(self, req: LoginRequest) -> LoginResponse:
        try:
            ...
            if check_password_hash(user.password, req.password):
                return LoginResponse(code=200, reason="", token=token)
            else:
                return LoginResponse(401, "invalid password", token=None)
        except Exception as e:
            return LoginResponse(
                code=500,
                reason=f"failed to log in: " + f"{type(e).__name__} {str(e)}",
                token=None,
            )
            # result = (
    ...
    def validate_token(self, req: ValidateTokenRequest) -> ValidateTokenResponse:
        try:
            ...
            data: Dict[str, int] = jwt.decode(
                req.token, os.environ["SECRET_KEY"], algorithms=["HS256"]
            )
            ...
            return ValidateTokenResponse(
                code=500,
                reason=f"failed to validate token: " + f"{type(e).__name__} {str(e)}",
            )
