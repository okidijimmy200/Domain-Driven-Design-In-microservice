...
def db_session():
    ...
    Table(
        ...
        Column("email", String(50), index=True, unique=True),
        Column("password", String(3000), index=True, nullable=False),
    )
...
def test_create_user(db_session):
    test_cases = [
        {"name": "pass", "input": MockUser, "output": (201, "user created")},
        {"name": "fail", "input": MockUser, "output": (403, "user already exists")},
    ]
...
def test_find_user(db_session):
    test_cases = [
        {"name": "pass", "input": "okidijimmie@gmail.com", "output": 200},
        {"name": "fail", "input": "test@test.com", "output": 404},
    ]
    for test_case in test_cases:
